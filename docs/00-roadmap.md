...
| Diagnostic « Ma plante a un problème » : photos + symptômes + observations (terre, racines, lumière, insectes) → pistes classées par vraisemblance avec gestes concrets (AI Services d'Infomaniak, modèle Mistral Small 4, clé de l'éditeur au build, sans plafond, jamais présenté comme certain), causes naturelles comprises — nectar extrafloral, guttation, vieille feuille qui finit — quand rien n'est anormal ; enregistrement dans le journal | ✅ AI Services d'Infomaniak (Mistral Small 4, clé au build, 30/jour) |
