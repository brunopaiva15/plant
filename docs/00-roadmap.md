...
| Comptes : Apple natif (iOS), Google natif (Android, Credential Manager) — pas d'e-mail ; compte local conservé sans connexion. Google sur Android attend son client OAuth (`AppConfig.googleWebClientId`, docs/20) ; Google par le navigateur sur iPhone reste derrière `AppConfig.googleSignInEnabled` | ✅ |
...
| Diagnostic « Ma plante a un problème » : photos + symptômes + observations (terre, racines, lumière, insectes) → pistes classées par vraisemblance avec gestes concrets (AI Services d'Infomaniak, modèle Qwen 3.5, clé de l'éditeur au build, sans plafond, jamais présenté comme certain), causes naturelles comprises — nectar extrafloral, guttation, vieille feuille qui finit — quand rien n'est anormal ; enregistrement dans le journal | ✅ AI Services d'Infomaniak (Qwen 3.5, clé au build, 30/jour) |
...
| Google Home (iOS et Android) : mêmes deux grandeurs sur les Home APIs, plateforme choisie avant la demande d'accès, capteurs des deux maisons dans la même liste. Livré sur iPhone ; le SDK se donne à la construction, et un build Android sans `-PgoogleHomeRepo` laisse la maison muette. Les Home APIs plafonnent à cent comptes tant que leur console développeur n'accepte pas d'inscription — la limite de leur beta publique | ✅ (iOS) |
...
| Achat unique au téléchargement (App Store) : tout inclus, aucun plafond ni fonction réservée, aucun achat intégré | ✅ |
...
- Relevé de la maison au LiDAR (RoomPlan, iPhone et iPad à LiDAR) : où poser une plante, par pièce, d'après la distance et l'orientation des fenêtres — livré (canal, modèle de lumière calibré sur le diorama, radiateurs, latitude, « Le jardin dans cette pièce », capteur de la pièce, emplacement renseigné, appartement entier par `StructureBuilder`, plantes placées sur le plan et « Placer ici », rideaux par fenêtre, balcon, section de sauvegarde) ; les deux premiers validés sur un iPhone Pro, le troisième attend son relevé réel ; la suite dans [docs/17-releve-de-la-maison.md](17-releve-de-la-maison.md)
- Import HortusFox (l'export complet JSON / CSV / ZIP est livré)
- Reconnaissance de plantes sur l'appareil, Pl@ntNet en repli : livrée, et le modèle embarqué en est à **Iris 9** (+3,4 points de top-1 sur Iris Indoor sur les photos de plantes cultivées, masque du lieu appliqué, et 949 espèces de plus qu'il sait nommer). Ce qu'il pèse, ce qu'il sait et ce qu'il vaut ne se recopient pas ici : la fiche est au [§ 0 de docs/09](09-plant-recognition.md#0-le-nom), recopiée de `assets/model/model.json`. Deux photos de la même plante valent quatorze points de top-1 — [docs/09-plant-recognition.md](09-plant-recognition.md). Ce qu'il reste à faire, et dans quel ordre : [§ 12](09-plant-recognition.md#12-ce-quil-reste-à-faire-dans-lordre) et [§ 13](09-plant-recognition.md#13-cadrage-de-liris-9--entraîner-large-exposer-étroit), et comment Indoor et Outdoor sont livrés ensemble au [§ 14](09-plant-recognition.md#14-livrer-indoor-et-outdoor-ensemble)
