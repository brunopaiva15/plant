...
| Google Home (iOS et Android) : mêmes deux grandeurs sur les Home APIs, plateforme choisie avant la demande d'accès, capteurs des deux maisons dans la même liste. Codé des deux côtés et vérifié sur un iPhone, pas ouvert (`AppConfig.googleHomeEnabled`) : les Home APIs plafonnent à cent comptes tant que la console développeur Google Home n'accepte pas d'inscription, et l'écran des capteurs garde la ligne « Google Home · Bientôt » | ⏳ (codé) |
