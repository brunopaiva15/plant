...
- Relevé de la maison au LiDAR (RoomPlan, iPhone Pro) : où poser une plante, par pièce, d'après la distance et l'orientation des fenêtres — les trois paliers codés derrière `--dart-define=ROOM_SCAN=true` (canal, modèle de lumière calibré sur le diorama, radiateurs, latitude, « Qui serait bien ici », capteur de la pièce, emplacement renseigné, appartement entier par `StructureBuilder`, plantes posées sur le plan et « Choisir cette place », rideaux par fenêtre, balcon, section de sauvegarde) ; les deux premiers validés sur un iPhone Pro, le troisième attend son relevé réel ; la suite dans [docs/17-releve-de-la-maison.md](17-releve-de-la-maison.md)
