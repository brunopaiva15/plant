...
/// Compte les diagnostics du jour, pour borner ce qu'un appareil peut
/// dépenser. La journée est un jour civil de l'appareil, en clair.
abstract class DiagnosisQuotaStore {
  String get day;
  int get count;
  Future<void> write({required String day, required int count});
}

/// Refuse au-delà de [limit] diagnostics par jour, avant tout appel réseau.
class DailyCappedDiagnoser implements PlantDiagnoser {
  const DailyCappedDiagnoser(this.inner, this.store, {required this.limit, this.today = _todayKey});

  final PlantDiagnoser inner;
  final DiagnosisQuotaStore store;
  final int limit;
  ...
  int get usedToday => store.day == today() ? store.count : 0;

  @override
  Future<Diagnosis> diagnose({required List<File> images, required String language, String? plantName, String? species, String? symptoms}) async {
    final day = today();
    final used = usedToday;
    if (used >= limit) throw const DiagnosisException('quota');
    final result = await inner.diagnose(images: images, language: language, plantName: plantName, species: species, symptoms: symptoms);
    await store.write(day: day, count: used + 1);
    return result;
  }
}
