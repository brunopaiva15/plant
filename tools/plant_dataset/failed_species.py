...
`--against` ajoute à la liste les espèces qui n'ont **aucune** ligne dans le
journal : une part tuée en route s'arrête sans rien écrire, et « 389/390 »
ne se distingue autrement de « 390/390 » qu'à l'œil. Les fichiers donnés
sont appariés aux journaux dans l'ordre.
...
#: Toute espèce dont le journal parle, quel qu'en soit le sort : gardées,
#: ÉCHEC, ou « nom non résolu chez GBIF ». Ce qui n'y figure pas n'a pas
#: été traité du tout.
SEEN = re.compile(r'^\[\d+/\d+\]\s+(?P<nom>.+?):\s')

#: Le motif porté par une espèce que le journal n'a jamais mentionnée.
NEVER = 'jamais traitée'
...
def never_reached(text: str, expected: list[str]) -> list[str]:
    """Les espèces attendues dont le journal ne parle pas du tout.

    Une part interrompue — machine qui s'endort, `nohup` qui tombe — laisse
    ses dernières espèces sans la moindre ligne. Elles ne sont pas en
    ÉCHEC : elles n'ont simplement jamais été essayées, et rien dans le
    journal ne les distingue de celles qui n'existaient pas.
    """
    vus = {m.group('nom').strip() for m in map(SEEN.match, (l.strip() for l in text.splitlines())) if m}
    return [n for n in expected if n not in vus]
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument('--against', action='append', metavar='FICHIER', default=[],
                    help='la liste d\'espèces confiée au journal correspondant ; ce qui n\'y a '
                         'aucune ligne est repris aussi. Répétable, apparié aux journaux dans l\'ordre.')
    ...
    if args.against and len(args.against) != len(args.logs):
        ap.error(f'{len(args.against)} liste(s) --against pour {len(args.logs)} journal/journaux : '
                 'elles s\'apparient dans l\'ordre, il en faut autant.')
    ...
    for i, chemin in enumerate(args.logs):
        ...
        rates = failures(texte)
        if args.against:
            attendues = [l.strip() for l in Path(args.against[i]).read_text(encoding='utf-8').splitlines() if l.strip()]
            connues = {n for n, _ in rates}
            rates += [(n, NEVER) for n in never_reached(texte, attendues) if n not in connues]
