"""Les étiquettes et la fusion des doublons des modèles de comparaison.

Pl@ntNet-300K a 1 081 sorties pour 1 019 espèces : la même plante y
...
def test_le_graphe_additionne_les_doublons():
    """Ce que rend `emballer` : une probabilité par espèce, les citations
    d'une même plante additionnées, et une somme qui vaut toujours 1."""
    torch = pytest.importorskip('torch')
    from comparaison import emballer

    class Logits(torch.nn.Module):
        def forward(self, x):
            return torch.log(torch.tensor([[0.1, 0.2, 0.3, 0.4]]))

    # Sorties 0 et 2 : la même espèce ; 1 et 3 chacune la leur.
    module = emballer(Logits(), [0, 1, 0, 2], 3)
    p = module(torch.zeros(1, 4, 4, 3))[0]
    assert torch.allclose(p, torch.tensor([0.4, 0.2, 0.4]))
