# ============================================================
# La scène 3D de l'icône d'Auxine, pour Blender (Cycles).
#
#   blender -b --python tool/app_icon_scene.py -- ÉCHANTILLONS SORTIE.png \
#       [FOND_HAUT FOND_BAS AMBIANCE]
#
# Ne se lance pas à la main d'ordinaire : tool/render_app_icon.py la rend
# calque par calque. Tout est construit ici, sans fichier .blend : un pot en
# terre cuite tourné depuis un profil, son terreau, une tige et deux feuilles
# d'argile mate, deux yeux, sur un dégradé sauge vu de la seule caméra. Après
# le rendu, un gain de saturation de 12 % puis un grain gaussien de 1,8 %, à
# graine fixe.
#
...
# CLIN : fermeture de l'œil de droite (vu de face), de 0 (ouvert) à 1 (fermé en arc)
CLIN=float(os.environ.get('CLIN','0'))
# FOND=0 rend le fond transparent ; OBJETS=0 ne rend que le fond
...
if YEUX:
    ...
    for x in (-EX,EX):
        ...
        clin=CLIN if x>0 else 0
        ys=-math.sqrt(rz*rz-x*x)-0.045
        if clin>=1:
            # fermé : un arc sombre, face à la caméra, aux bouts arrondis
            cu=bpy.data.curves.new('paupiere','CURVE');cu.dimensions='3D';cu.bevel_depth=.16*ER;cu.bevel_resolution=8
            sp=cu.splines.new('BEZIER');sp.bezier_points.add(2)
            for p,co in zip(sp.bezier_points,((-0.85*ER,0,-0.22*ER),(0,0,0.30*ER),(0.85*ER,0,-0.22*ER))):
                p.co=co;p.handle_left_type=p.handle_right_type='AUTO'
            a=obj('paupiere',cu,eyem);a.location=(x,ys,EZ);a.rotation_euler=(CAM-a.location).to_track_quat('-Y','Z').to_euler()
            for bx,bz in ((-0.85,-0.22),(0.85,-0.22)):
                bpy.ops.mesh.primitive_uv_sphere_add(radius=.16*ER,location=(bx*ER,0,bz*ER));b=bpy.context.object
                b.data.materials.append(eyem);bpy.ops.object.shade_smooth();b.parent=a
            bm.free();continue
        for dx,dz,sx,sz,rot in (((-0.34,0.40,0.30,0.20,-35),(0.30,-0.42,0.10,0.08,0)) if clin<=0.5 else ()):
        ...
        e.location=(x,ys,EZ);e.scale=(ER,.42*ER,1.12*ER*(1-clin))
