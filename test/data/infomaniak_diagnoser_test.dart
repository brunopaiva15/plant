...
void main() {
  ...
  group('le plafond journalier', () {
    test('compte les diagnostics du jour et refuse au-delà', () async {
      final store = _MemoryQuota();
      var calls = 0;
      final inner = _FakeDiagnoser(() => calls++);
      final capped = DailyCappedDiagnoser(inner, store, limit: 2, today: () => '2026-09-06');
      final tmp = await _tmpImage();
      await capped.diagnose(images: [tmp], language: 'fr');
      await capped.diagnose(images: [tmp], language: 'fr');
      expect(store.count, 2);
      await expectLater(capped.diagnose(images: [tmp], language: 'fr'), throwsA(predicate((e) => e is DiagnosisException && e.message == 'quota')));
      expect(calls, 2, reason: 'le refus a lieu avant tout appel');
      await tmp.delete();
    });

    test('repart de zéro le lendemain', () async {
      final store = _MemoryQuota()
        ..day = '2026-09-05'
        ..count = 30;
      final capped = DailyCappedDiagnoser(_FakeDiagnoser(() {}), store, limit: 30, today: () => '2026-09-06');
      expect(capped.usedToday, 0);
      final tmp = await _tmpImage();
      await capped.diagnose(images: [tmp], language: 'fr');
      await tmp.delete();
      expect(store.day, '2026-09-06');
      expect(store.count, 1);
    });
  });
}

class _MemoryQuota implements DiagnosisQuotaStore {
  @override
  String day = '';
  @override
  int count = 0;
...
class _FakeDiagnoser implements PlantDiagnoser {
  _FakeDiagnoser(this.onCall);
  ...
  Future<Diagnosis> diagnose({required List<File> images, required String language, String? plantName, String? species, String? symptoms}) async {
    onCall();
    return const Diagnosis(summary: 'ok', causes: []);
  }
}
