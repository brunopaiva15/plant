...
Future<void> _reachResults(WidgetTester tester, {String spot = 'Coin sombre', String effort = "J'oublie souvent d'arroser", String safety = 'Oui, sans risque de préférence'}) async {
  await _pump(tester);
...
void main() {
  group('« Trouver une plante »', () {
    testWidgets('trois questions, puis un premier choix et ses raisons', (tester) async {
      await _pump(tester);
      expect(find.text('Où va-t-elle vivre ?'), findsOneWidget);
    ...
    testWidgets('le genre de plante filtre les propositions sur place', (tester) async {
      ...
      expect(find.text('Rien de convaincant'), findsOneWidget);
